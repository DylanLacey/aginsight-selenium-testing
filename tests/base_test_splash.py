...
from UIComponentObjects import Tour, Footer, Header

'''All test modules inherit from the BaseTestSplash class.'''
...
class BaseTestSplash(unittest.TestCase):
    username = None
    access_key = None
    selenium_port = None
    selenium_host = None
    ...
    def setUp(self):
        ...
        if BaseTestSplash.tunnel_id:
            self.desired_capabilities['tunnel-identifier'] = BaseTestSplash.tunnel_id
        if BaseTestSplash.build_tag:
            self.desired_capabilities['build'] = BaseTestSplash.build_tag

        self.driver = webdriver.Remote(
                command_executor="http://%s:%s@%s:%s/wd/hub" %
                                 (BaseTestSplash.username,
                                  BaseTestSplash.access_key,
                                  BaseTestSplash.selenium_host,
                                  BaseTestSplash.selenium_port),
                desired_capabilities=self.desired_capabilities)

        # Open the website in the BaseTestSplash, this can now be used in multiple tests, without
    ...
    def tearDown(self):
        self.driver.quit()
        sauce_client = SauceClient(BaseTestSplash.username, BaseTestSplash.access_key)
